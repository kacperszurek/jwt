...
COPY . .
RUN chmod +x /szkolenie/tools/jwt_tool/jwt_tool.py && chmod +x /szkolenie/tools/rsa_sign2n/jwt_forgery.py && ln -s /szkolenie/tools/jwt_tool/jwt_tool.py /usr/local/bin/jwt_tool && ln -s /szkolenie/tools/rsa_sign2n/jwt_forgery.py /usr/local/bin/jwt_forgery
CMD ["/bin/sh"]
